     ...
     CopyHostMatrixToKernelMatrix&& copyHostMatrixToKernelMatrix, GetMatrixInfo&& getMatrixInfo)
{
  ...
  auto generateInputHostMatrix = [](const Coordinates& coords)
  {
    ...
    {
      math::ComplexMatrix* hinput = oap::chost::NewReMatrix (1, 3);
  ...
  auto generateOutputHostMatrix = [](uintt count, uintt rows = 1)
  {
    ...
    for (size_t idx = 0; idx < count; ++idx)
    {
      math::ComplexMatrix* houtput = oap::chost::NewReMatrix (1, rows);
  ...
  auto generateExpectedHostMatrix = [](const Coordinates& coords)
  {
    ...
    for (size_t idx = 0; idx < coords.size(); ++idx)
    {
      math::ComplexMatrix* hexpected = oap::chost::NewReMatrix (1, 1);
      const auto& coord = coords[idx];
...

  {
    ...
    auto createMMLayer = [&network] (LHandler handler, uintt startIdx, const Coordinates& coords, uintt length)
    {
      ...
      oap::chost::deleteMatrices (hinputs);
      oap::chost::deleteMatrices (houtputs);
    };
    ...
    oap::HostComplexMatrixPtr hinput = oap::chost::NewReMatrix (1, 3);
    ...
    while (testError > 0.005 && terrorCount < 360);

    EXPECT_GE (357, terrorCount);

    //oap::pyplot::plotCoords2D ("/tmp/plot_plane_xy.py", std::make_tuple(-5, 5, 0.1), std::make_tuple(-5, 5, 0.1), getLabel, {"r*", "b*"});
    oap::chost::deleteMatrices(testHInputs);
    oap::chost::deleteMatrices(trainingHInputs);

    oap::chost::deleteMatrices(testHOutputs);
    oap::chost::deleteMatrices(trainingHOutputs);

    oap::chost::deleteMatrices(testHExpected);
    oap::chost::deleteMatrices(trainingHExpected);
  }
}
