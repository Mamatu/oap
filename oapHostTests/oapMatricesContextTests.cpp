...
#include "oapMatricesContext.hpp"
...
TEST_F(OapMatricesContextTests, ReturnUnusedMatrix_ApiTest_1)
{
  ...
  context.registerMemType (HOST, oap::chost::NewHostMatrixFromMatrixInfo, oap::chost::DeleteComplexMatrix);
  context.registerMemType ("HOST_1", oap::chost::NewHostMatrixFromMatrixInfo, oap::chost::DeleteComplexMatrix);

  math::MatrixInfo minfo (true, false, 10, 10);

  math::ComplexMatrix* m1 = context.useMatrix (minfo, HOST);
  context.unuseAllMatrices ();
  math::ComplexMatrix* m2 = context.useMatrix (minfo, HOST);
...
TEST_F(OapMatricesContextTests, ReturnUnusedMatrix_ApiTest_2)
{
  ...
  math::ComplexMatrix* m1 = context.useMatrix (minfo, "HOST");
  context.unuseAllMatrices ();
  math::ComplexMatrix* m2 = context.useMatrix (math::MatrixInfo (true, false, 10, 10), "HOST");
...
TEST_F(OapMatricesContextTests, Clear_ApiTest)
{
  ...
  {
    public:
      virtual math::ComplexMatrix* newMatrix (const math::MatrixInfo& minfo)
      {
  ...
  {
    public:
      MOCK_METHOD1 (newMatrix, math::ComplexMatrix*(const math::MatrixInfo&));
  };
  
  
  AllocatorMock allocatorMock;
  EXPECT_CALL (allocatorMock, newMatrix(_)).WillOnce (Invoke (oap::chost::NewHostMatrixFromMatrixInfo));

  context.registerMemType (HOST, std::bind (&AllocatorMock::newMatrix, &allocatorMock, std::placeholders::_1), oap::chost::DeleteComplexMatrix);
...
TEST_F(OapMatricesContextTests, GetterInMethod_ApiTest)
{
  ...
  AllocatorMock allocatorMock;
  EXPECT_CALL (allocatorMock, newMatrix (_)).Times (2).WillRepeatedly(Invoke (oap::chost::NewHostMatrixFromMatrixInfo));
...
TEST_F(OapMatricesContextTests, GeneralTest_1)
{
  ...
  context.registerMemType ("CUDA", oap::chost::NewHostMatrixFromMatrixInfo, oap::chost::DeleteComplexMatrix);

  auto func = [&context, &minfo](math::ComplexMatrix** m1)
  {
    auto getter = context.getter();
    math::ComplexMatrix* matrix = getter.useMatrix (minfo, "CUDA");
    *m1 = matrix;
  };

  math::ComplexMatrix* m01;
  math::ComplexMatrix* m11;
...
TEST_F(OapMatricesContextTests, GeneralTest_2)
{
  ...
  auto func = [&context, &minfo](math::ComplexMatrix** m1, math::ComplexMatrix** m2)
  {
    auto getter = context.getter();
    math::ComplexMatrix* matrix = getter.useMatrix (minfo, "CUDA");
    math::ComplexMatrix* matrix1 = getter.useMatrix (minfo, "CUDA");
    *m1 = matrix;
    *m2 = matrix1;
  };

  math::ComplexMatrix* m1[2];
  math::ComplexMatrix* m2[2];
