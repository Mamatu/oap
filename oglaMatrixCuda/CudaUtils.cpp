...
void SetReValue(math::Matrix* m, uintt index, floatt value) {
    ...
    cuMemcpyHtoD(reinterpret_cast<CUdeviceptr> (&array[index]), &value, sizeof (floatt));
}

floatt GetReValue(math::Matrix* m, uintt index) {
    floatt* array = GetReValues(m);
    floatt value = 0;
    cuMemcpyDtoH(&value, reinterpret_cast<CUdeviceptr> (&array[index]), sizeof (floatt));
    return value;
}
...
void SetZeroRow(math::Matrix* matrix, uintt index, bool re, bool im) {
    ...
    if (NULL != rearray && re) {
        cuMemsetD32(reinterpret_cast<CUdeviceptr> (&rearray[columns * index]), 0, elementsCount);
    }
    if (NULL != imarray && im) {
        cuMemsetD32(reinterpret_cast<CUdeviceptr> (&imarray[columns * index]), 0, elementsCount);
    }
}
