...
void CuMatrix::magnitude2OptVer2(floatt& output, math::Matrix* params0) {
    ...
    const uintt h = CudaUtils::GetRows(params0);
    if (w > 1) {
        output = magnitude2Procedure("CUDAKernel_MagitudeOptVer2", params0, w / 2, h);
    } else {
        output = magnitude2Procedure("CUDAKernel_MagitudeOpt", params0, w, h);
    }
}
...
bool CuMatrix::compareVer2(math::Matrix* matrix1, math::Matrix* matrix2) {
    ...
    return compareProcedure("CUDAKernel_CompareOptVer2", matrix1, matrix2, w, h, w / 2, h);
}

bool CuMatrix::compareProcedure(const char* cuKernelName,
    ...
    m_cuResult = ::cuda::Kernel::Execute(cuKernelName, params,
        m_kernel, m_image);
...
floatt CuMatrix::magnitude2Procedure(const char* cuKernelName,
    math::Matrix* matrix, uintt wthreads, uintt hthreads) {
    ...
    assert(threads[0] * threads[1] * sizeof (floatt) < m_kernel.getSharedMemorySize());
