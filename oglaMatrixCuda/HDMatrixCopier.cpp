...
void HDMatrixCopier::copyMatrixToMatrix(math::Matrix* dst, const math::Matrix* src) {
    uintt length1 = getDeviceColumns(dst) * getDeviceRows(dst);
    uintt length2 = src->columns * src->rows;
    length1 = length1 < length2 ? length1 : length2;
    CUdeviceptr dstRePtr = reinterpret_cast<CUdeviceptr>(getReValues(dst));
    CUdeviceptr dstImPtr = reinterpret_cast<CUdeviceptr>(getImValues(dst));
