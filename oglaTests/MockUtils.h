  ...
  for (uintt fa = 0; fa < m1.columns; ++fa) {
    for (uintt fb = 0; fb < m1.rows; ++fb) {
      if (m1.reValues != NULL && m2.reValues != NULL &&
          m1.reValues[fa + fb * m1.columns] !=
              m2.reValues[fa + fb * m2.columns]) {
        return false;
      }

      if (m1.imValues != NULL && m2.imValues != NULL &&
          m1.imValues[fa + fb * m1.columns] !=
              m2.imValues[fa + fb * m2.columns]) {
        return false;
      }
    }
  }
...
MATCHER_P(MatrixIsEqual, matrix1, "") { return (*arg) == (*matrix1); }
