...
#define CALL_RET(func,...) return RpcBase::func(__VA_ARGS__);
    ...
    void Function_impl(utils::Reader& input, utils::Writer* outputs, void * ptr) {
        utils::OglaFunction* function = (utils::OglaFunction*)(ptr);
        if (outputs) {
    ...
    LHandle RpcImpl::registerCall(utils::OglaFunction* function) {
        const char* functionName = function->getName();
        const char** names = NULL;
        int namesCount = 0;
        OglaObject::ConvertToStrings(function->getRoot(), &names, namesCount);
