...
#define CONVERT_BUFFER_SIZE 256
    ...
    utils::Socket* RpcBase::getSocket(LHandle clientID) {
        if (this->addresses.find(clientID) == this->addresses.end()) {
            return NULL;
        }
        return reinterpret_cast<utils::Socket*> (clientID.getPtr());
    }
    ...
    void RpcBase::ServerExecution(Socket* server, Socket* client, const char* buffer, int size, void* data_ptr) {
        RpcBase* rpc = (RpcBase*) data_ptr;
        SocketsWriters::iterator it = rpc->socketsWriters.find(client);
        if (it == rpc->socketsWriters.end()) {
            Writer* w = new Writer();
            char* convertBuffer = new char[CONVERT_BUFFER_SIZE];
            RpcBase::SocketData socketData = {w, convertBuffer};
            rpc->socketsWriters[client] = socketData;
        }
        SocketData sd = rpc->socketsWriters[client];
            ...
            while (1) {
                ...
                if (magicNumber == RpcBase::MAGIC_NUMBER) {
                    ...
                    if (sectionSize <= reader.getSize() - reader.getPosition()) {
                        ...
                        debug("type == %u \n", type);
                        RpcBase::ServerSubExecution(rpc, type, reader, data_ptr);
    ...
    void RpcBase::ServerSubExecution(RpcBase* rpc, int type, Reader& reader, void* ptr) {
    ...
    void RpcBase::sendFunctionEvent(LHandle actionID, LHandle connectionID) {
        ...
        Socket* socket = this->getSocket(connectionID);
        socket->send(&writer);
    }
    ...
    LHandle RpcBase::waitOnConnection() {
        this->initID = 0;
        if (server == NULL) {
            server = utils::Socket::CreateServer(port, ServerExecution, this);
        }
    ...
    LHandle RpcBase::connect(const char* address, uint16_t port) {
        ...
        if (clientSocket) {
            ...
            if (server == NULL) {
                server = utils::Socket::CreateServer(this->port, RpcBase::ServerExecution, this);
            }
            server->connect();
    ...
    LHandle RpcBase::establishConnection(const char* address, uint16_t port) {
        if (server != NULL) {
            debug("OK 1\n");
            utils::Socket* client = utils::Socket::CreateClient(address, port);
    ...
    void RpcBase::call(LHandle connectionID, LHandle functionID, const char** functionNames, uint namesCount,
        ...
        if (functionNames != NULL) {
            utils::Socket* socket = this->getSocket(connectionID);
            if (socket) {
    ...
    void RpcBase::InvokeFunction(LHandle actionID, LHandle connectionID, Reader& reader, RpcBase* rpc) {
        ...
        Socket* clientSocket = rpc->getSocket(connectionID);
        if (clientSocket) {
            clientSocket->send(&writer1);
        }
    }
    ...
    void RpcBase::EstablishConnection(LHandle actionID, LHandle remoteClientID, Reader& reader, RpcBase* rpc) {
        ...
        Socket* clientSocket = rpc->getSocket(clientID);
        Writer writer1;
        writer1.write(RpcBase::MAGIC_NUMBER);
        writer1.write(&writer);
        if (clientSocket) {
