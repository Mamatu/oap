...
int main(int argc, char** argv) {
    ...
    host::PrintReMatrix(h1);
    math::Matrix* h2 = host::NewReMatrixCopy(4, 4, (floatt*) b);
    host::PrintReMatrix(h2);

    transferMatrixCpu.setHamiltonian1(h1, shibata::ORIENTATION_REAL_DIRECTION);
    transferMatrixCpu.setHamiltonian2(h2, shibata::ORIENTATION_REAL_DIRECTION);
    int threadsCount = 1;
    int spinsCount = 3; //25;
    int q[] = {-1, 1};
    int qCount = 2;
    ...
    int status = transferMatrixCpu.start();
    if (status == 0) {
        host::PrintReMatrix(stderr, transferMatrix);
    } else {
        //    printf("ERROR: status == %d \n", status);
    }


    IraMethod iram(transferMatrix, transferMatrix->columns, 1);
    iram.start();

    hmm.deleteMatrix(temp);
    hmm.deleteMatrix(transferMatrix);
    hmm.deleteMatrix(spin1);
    hmm.deleteMatrix(spin2);
    hmm.deleteMatrix(spin3);
    hmm.deleteMatrix(spin4);
    hmm.deleteMatrix(h1);
    hmm.deleteMatrix(h2);
    return 0;
}
