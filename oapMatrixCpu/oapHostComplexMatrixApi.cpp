...
{
  math::ComplexMatrix* output = new math::ComplexMatrix();
...
math::ComplexMatrix* NewComplexMatrixRef (const math::ComplexMatrix* matrix)
{
  ...
  {
    output = NewComplexMatrix(gColumns (matrix), gRows (matrix));
  }
...
math::ComplexMatrix* NewComplexMatrix(const math::ComplexMatrix* matrix, uintt columns, uintt rows)
{
  ...
  if (gReValues (matrix) != nullptr && gImValues (matrix) != nullptr)
  {
    output = NewComplexMatrix(columns, rows);
  }
  else if (gReValues (matrix) != nullptr)
  {
...
{
  math::ComplexMatrix* matrix = NewComplexMatrix (columns, rows);
  oap::chost::CopyArrayToMatrix (matrix, rearray, imarray);
  return matrix;
}
...
math::ComplexMatrix* NewReMatrixCopyOfArray (uintt columns, uintt rows, const floatt* rearray)
{
  ...
  oap::chost::CopyArrayToReMatrix (matrix, rearray);
  return matrix;
}
...
math::ComplexMatrix* NewImMatrixCopyOfArray (uintt columns, uintt rows, const floatt* imarray)
{
  ...
  oap::chost::CopyArrayToImMatrix (matrix, imarray);
  return matrix;
}

math::ComplexMatrix* NewComplexMatrix(const math::MatrixInfo& matrixInfo)
{
  return NewComplexMatrix(matrixInfo.isRe, matrixInfo.isIm, matrixInfo.columns (), matrixInfo.rows ());
}
...
math::ComplexMatrix* NewComplexMatrix (bool isre, bool isim, uintt columns, uintt rows)
{
  if (isre && isim)
  {
    return oap::chost::NewComplexMatrix(columns, rows);
  }
  else if (isre)
  {
    return oap::chost::NewReMatrix(columns, rows);
  }
  else if (isim)
  {
    return oap::chost::NewImMatrix(columns, rows);
  }
  return nullptr;
}
...
{
  return oap::chost::NewComplexMatrixWithValue (minfo.isRe, minfo.isIm, minfo.columns (), minfo.rows (), value);
}
...
math::ComplexMatrix* NewComplexMatrixWithValue (bool isre, bool isim, uintt columns, uintt rows, floatt value)
{
  if (isre && isim)
  {
    return oap::chost::NewComplexMatrixWithValue (columns, rows, value);
  }
  else if (isre)
  {
    return oap::chost::NewReMatrixWithValue (columns, rows, value);
  }
  else if (isim)
  {
    return oap::chost::NewImMatrixWithValue (columns, rows, value);
  }
  return nullptr;
}
...
{
  std::string output;
  oap::generic::printMatrix (output, matrix, matrixUtils::PrintArgs(), oap::chost::GetMatrixInfo);
  return output;
}
...
{
#if 0
  oap::generic::MatrixMemoryApi<decltype(oap::chost::GetMatrixInfo), decltype (oap::chost::ToHost)> mmApi (oap::chost::GetMatrixInfo, oap::chost::ToHost);
...
{
  math::ComplexMatrix* output = oap::chost::NewComplexMatrixRef (matrix);
  oap::chost::CopyMatrix(output, matrix);
  return output;
}
...
void ToString (std::string& str, const math::ComplexMatrix* matrix)
{
  ...
  oap::generic::printMatrix (str, matrix, args, oap::chost::GetMatrixInfo);
}
...
{
  oap::chost::SetDiagonalReMatrix(matrix, 1);
  oap::chost::SetImZero(matrix);
}
...
math::ComplexMatrix* ReadMatrix (const std::string& path)
{
  ...
  math::ComplexMatrix* matrix = oap::chost::LoadMatrix (buffer);
...
math::ComplexMatrix* ReadRowVector (const std::string& path, size_t index)
{
  ...
  math::ComplexMatrix* subMatrix = oap::chost::NewSubMatrix (matrix, 0, index, gColumns (matrix), 1);
  return subMatrix;
}
...
{
  oap::HostComplexMatrixUPtr matrix = ReadMatrix (path);
  math::ComplexMatrix* subMatrix = oap::chost::NewSubMatrix (matrix, index, 0, 1, gRows (matrix));
  return subMatrix;
}
...
bool WriteMatrix (const std::string& path, const math::ComplexMatrix* matrix)
{
  ...
  oap::chost::SaveMatrix (matrix, buffer);
  try
  {
    buffer.fwrite (path);
  }
...
math::ComplexMatrix* NewSubMatrix (const math::ComplexMatrix* orig, uintt cindex, uintt rindex, uintt clength, uintt rlength)
{
  ...
  math::ComplexMatrix* submatrix = oap::chost::NewComplexMatrix (orig, clength, rlength);
...
void SaveMatrix (const math::ComplexMatrix* matrix, utils::ByteBuffer& buffer)
{
  ...
  auto minfo = oap::chost::GetMatrixInfo (matrix);
...
math::ComplexMatrix* LoadMatrix (const utils::ByteBuffer& buffer)
{
  ...
  math::ComplexMatrix* matrix = NewComplexMatrix (minfo);
...
{
  oap::generic::setMatrix (matrix, matrix1, column, row, [](math::ComplexMatrix* matrix) { return matrix->re.mem; }, [](math::ComplexMatrix* matrix) { return matrix->re.reg; }, memcpy, memmove);
}

void SetImMatrix (math::ComplexMatrix* matrix, math::ComplexMatrix* matrix1, uintt column, uintt row)
{
  oap::generic::setMatrix (matrix, matrix1, column, row, [](math::ComplexMatrix* matrix) { return matrix->im.mem; }, [](math::ComplexMatrix* matrix) { return matrix->im.reg; }, memcpy, memmove);
}
...
math::ComplexMatrix* NewComplexMatrixFromMemory (uintt columns, uintt rows, oap::Memory& remem, const oap::MemoryLoc& reloc, oap::Memory& immem, const oap::MemoryLoc& imloc)
{
...
void SetReZeroRow (const math::ComplexMatrix* matrix, uintt index)
{
  ...
  if (hm.re.mem.ptr)
  {
    ...
    oap::generic::copy (hm.re.mem.ptr, hm.re.mem.dims, loc, row.data(), {1, columns}, {{0, 0}, {1, columns}}, memcpy, memmove);
  }
}
...
void SetImZeroRow (const math::ComplexMatrix* matrix, uintt index)
{
  ...
  if (hm.im.mem.ptr)
  {
    ...
    oap::generic::copy (hm.im.mem.ptr, hm.im.mem.dims, loc, row.data(), {1, columns}, {{0, 0}, {1, columns}}, memcpy, memmove);
  }
}
...
{
  return oap::generic::getDiagonal (matrix, index, oap::chost::GetRefHostMatrix,
                                    [](const math::ComplexMatrix* matrix, const math::ComplexMatrix& ref){return matrix->re.mem;},
                                    [](const math::ComplexMatrix* matrix, const math::ComplexMatrix& ref){return matrix->re.reg;}, memcpy, memmove);
}
...
floatt GetImDiagonal (const math::ComplexMatrix* matrix, uintt index)
{
  return oap::generic::getDiagonal (matrix, index, oap::chost::GetRefHostMatrix,
                                    ...
                                    [](const math::ComplexMatrix* matrix, const math::ComplexMatrix& ref){return matrix->im.reg;}, memcpy, memmove);
}

void CopyReMatrixToHostBuffer (floatt* buffer, uintt length, const math::ComplexMatrix* matrix)
{
  math::ComplexMatrix ref = oap::chost::GetRefHostMatrix (matrix);
...
std::string to_carraystr(const std::vector<math::ComplexMatrix*>& matrices)
{
  ...
  {
    strs.push_back (oap::chost::to_carraystr (matrix));
  }
  std::stringstream sstream;
  sstream << "{";
...
std::vector<math::ComplexMatrix*> NewMatrices (const std::vector<math::MatrixInfo>& minfos)
{
  ...
  {
    math::ComplexMatrix* matrix = oap::chost::NewHostMatrixFromMatrixInfo (minfo);
...
std::vector<math::ComplexMatrix*> NewMatricesCopyOfArray (const std::vector<math::MatrixInfo>& minfos, const std::vector<std::vector<floatt>>& arrays)
{
  ...
  for (uintt idx = 0; idx < minfos.size(); ++idx)
  {
    ...
    {
      math::ComplexMatrix* matrix = NewComplexMatrixCopyOfArray (minfo.columns(), minfo.rows(), arrays[idx].data(), arrays[idx].data());
      matrices.push_back (matrix);
    }
    else if(minfo.isRe)
    {
...
math::ComplexMatrix* NewSharedSubMatrix (const math::MatrixLoc& loc, const math::MatrixDim& dim, const math::ComplexMatrix* matrix)
{
  ...
  math::ComplexMatrix refmatrix = oap::chost::GetRefHostMatrix (matrix);
  math::ComplexMatrix* output = nullptr;
  ...
  if (minfo.isRe && minfo.isIm)
  {
    ...
    output = oap::chost::NewComplexMatrixFromMemory (dim.columns, dim.rows, refmatrix.re.mem, reloc, refmatrix.im.mem, imloc);
  }
  else if (minfo.isRe)
  {
    oap::MemoryLoc reloc = oap::common::ConvertRegionLocToMemoryLoc (refmatrix.re.mem, refmatrix.re.reg, {loc.x, loc.y});
    output = oap::chost::NewReMatrixFromMemory (dim.columns, dim.rows, refmatrix.re.mem, reloc);
  }
  else if (minfo.isIm)
  {
    oap::MemoryLoc imloc = oap::common::ConvertRegionLocToMemoryLoc (refmatrix.im.mem, refmatrix.im.reg, {loc.x, loc.y});
    output = oap::chost::NewImMatrixFromMemory (dim.columns, dim.rows, refmatrix.im.mem, imloc);
  }
