...
#ifndef OAP_HOST_COMPLEX_MATRIX_API_HPP
#define OAP_HOST_COMPLEX_MATRIX_API_HPP
...
{
  math::ComplexMatrix* output = oap::chost::NewComplexMatrix(reArray != NULL, imArray != NULL, columns, rows);
  oap::chost::Copy<T>(output, reArray, imArray);
  return output;
}
...
{
  math::ComplexMatrix* output = oap::chost::NewComplexMatrix(reArray != NULL, false, columns, rows);
  oap::chost::CopyRe<T>(output, reArray);
  return output;
}
...
{
  math::ComplexMatrix* output = oap::chost::NewComplexMatrix(false, imArray != NULL, columns, rows);
  oap::chost::CopyIm<T>(output, imArray);
  return output;
}
...
{
  return oap::threads::createThreadsMapper (matricesVec, oap::chost::GetRefHostMatrix, malloc, memcpy, free, algo);
}
