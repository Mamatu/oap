...
inline void createUpIndecies(intt row, int columns, intt qc, intt* pows,
                             int chainLength, intt** columnsIndecies,
                             intt& columnsSize) {
...
inline void createDownIndecies(intt column, int rows, intt qc, intt* pows,
                               int chainLength, intt** rowsIndecies,
                               intt& rowsSize) {
  intt sp1 = column % qc;
...
int RealTransferMatrix::getTMIndex(char** spinValueIndex, int posIndex,
                                   int timeLimit, int qc) {
  int index = 0;
...
int RealTransferMatrix::getIndex(char** spinValueIndex, int posIndex,
                                 int timeIndex, int qc, int posLimit,
                                 int timeLimit) {
...
bool RealTransferMatrix::isZero(std::vector<std::pair<int, int> >& vector,
...
int RealTransferMatrix::getIndex(char* spinValueIndex, int spin1Index, int qc,
                                 int count) {
  int output = 0;
...
int RealTransferMatrix::getSpinsCount() { return 3; }

int RealTransferMatrix::getVirtualTime() {
  return this->parameters.getTrotterNumber() * 2;
}

void RealTransferMatrix::transformMatrixOrientation(
    math::Matrix* dst, math::Matrix* src, shibataCpu::Orientation orientation) {
  if (orientation == shibataCpu::ORIENTATION_REAL_DIRECTION) {
    int spinsCount = src->rows;
    int qc = this->parameters.getQunatumsCount();
    tempSpins =
        tempSpins == NULL ? ArrayTools::create<char>(spinsCount, 0) : tempSpins;
    ArrayTools::clear<char>(tempSpins, spinsCount, 0);
    do {
      floatt rvalue = host::GetReValue(src, tempSpins[0] + qc * tempSpins[1],
                                       tempSpins[2] + qc * tempSpins[3]);
      floatt ivalue = host::GetImValue(src, tempSpins[0] + qc * tempSpins[1],
                                       tempSpins[2] + qc * tempSpins[3]);
      host::SetReValue(dst, tempSpins[2] + qc * tempSpins[0],
                       tempSpins[3] + qc * tempSpins[1], rvalue);
      host::SetImValue(dst, tempSpins[2] + qc * tempSpins[0],
                       tempSpins[3] + qc * tempSpins[1], ivalue);
    } while (ArrayTools::increment<char>(tempSpins, 0, qc, 1, 0, spinsCount) <
             spinsCount);
  }
}

math::Status RealTransferMatrix::onExecute() { return math::STATUS_OK; }
...
void RealTransferMatrix::alloc(::device::Kernel& kernel) {
  if (m_isAllocated == false) {
    ...
    tmColumnsPtr =
        (intt**)CudaUtils::AllocDeviceMem(threadsCount * sizeof(intt*));
    tmRowsPtr = (intt**)CudaUtils::AllocDeviceMem(threadsCount * sizeof(intt*));
    tmRowsBitsPtr =
        (char**)CudaUtils::AllocDeviceMem(threadsCount * sizeof(char*));

    treePointers = (TreePointer***)CudaUtils::AllocDeviceMem(
        threadsCount * sizeof(TreePointer**));
    ...
    for (uintt fa = 0; fa < threadsCount; ++fa) {
      ...
      CudaUtils::CopyHostToDevice(&tmRowsBitsPtr[fa], &tmRowsBits,
                                  sizeof(char*));

      TreePointer** treePointers1 =
          (TreePointer**)CudaUtils::AllocDeviceMem(M2 * sizeof(TreePointer*));
      ...
      for (intt fb = 0; fb < M2; ++fb) {
        TreePointer* treePointer =
            m_treePointerCreator.create(fb, expHamiltonian1, expHamiltonian2);
    ...
    dupIndecies = (intt*)CudaUtils::AllocDeviceMem(
        upIndeciesCount * sizeof(intt), upIndecies);
    ddownIndecies = (intt*)CudaUtils::AllocDeviceMem(
        downIndeciesCount * sizeof(intt), downIndecies);
    ...
    uintt rows = CudaUtils::GetColumns(expHamiltonian1);
    uintt columns = CudaUtils::GetColumns(expHamiltonian2);
    for (intt fa = 0; fa < rows; fa++) {
      createDownIndecies(fa, columns, quantumsCount, pows, 2, &downIndecies,
                         downIndeciesCount);
    }
    for (intt fa = 0; fa < columns; fa++) {
      createUpIndecies(fa, rows, quantumsCount, pows, 2, &upIndecies,
                       upIndeciesCount);
    }
    m_isAllocated = true;
  }
...
math::Status RealTransferMatrix::execute() {
  ...
  kernel.load("liboapShibataCuda.cubin");
  alloc(kernel);
  if (transferMatrix) {
    void* params[] = {&m_output, &treePointers, &tmColumnsPtr, &tmRowsPtr,
                      &tmRowsBitsPtr, &dupIndecies, &ddownIndecies,
                      &ddownIndeciesCount, &dquantumsCount, &dM2, &dwidth};
    kernel.setParams(params);
    kernel.load(
        "/home/mmatula/Oap/oapShibataCuda/\
dist/Debug/GNU-Linux-x86/liboapShibataCuda.cubin");
    kernel.execute("ExecuteTM");
  } else {
    void* params[] = {&m_reoutputEntries, &m_imoutputEntries, &m_entries,
                      &m_entriesCount, &treePointers, &tmColumnsPtr, &tmRowsPtr,
                      &tmRowsBitsPtr, &dupIndecies, &ddownIndecies,
                      &ddownIndeciesCount, &dquantumsCount, &dM2, &dwidth};
    kernel.setParams(params);
    kernel.load(
